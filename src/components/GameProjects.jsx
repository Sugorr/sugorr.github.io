...
export default function GameProjects() {
    ...
    return (
        <section ref={targetRef} className="relative h-[200vh] items-center bg-default-black rounded-3xl">
            <motion.div ref={textRef} className="sticky top-0 flex h-screen items-center overflow-hidden p-32">
                <motion.div style={{ x }} className="h-full flex py-32 gap-12 items-center">
                    <motion.div className="h-full w-full px-32 flex flex-col justify-between items-start">
                        <motion.div
                        initial={{ y: 20, opacity: 0 }} 
                        animate={{ y: textInView ? 0 : 20, opacity: textInView ? 1 : 0, transition: {duration: 0.5}}} 
                        className="text-[4dvw] font-bold text-default-white">Game Projects</motion.div>
                        <motion.div 
                        initial={{ x: -20, opacity: 0 }} 
                        animate={{ x: textInView ? 0 : -20, opacity: textInView ? 1 : 0, transition: {duration: 0.8}}} 
                        className="text-3xl text-default-white/50 font-semibold">Building <span className="text-default-orange">games</span>, fueled by fun and creativity!</motion.div>
                        <motion.a
                        initial={{ x: 20, opacity: 0 }} 
                        animate={{ x: textInView ? 0 : 20, opacity: textInView ? 1 : 0, transition: {duration: 1}}} 
                        href="https://sugordev.itch.io" target="_blank" className="text-2xl text-default-white bg-default-orange drop-shadow-sm rounded-xl px-10 py-2">
                            See All
                        </motion.a>
                    </motion.div>
                    {games.map((game) => (
...
const Game = ({ game }) => {
    return (
        <div className={`group flex flex-col items-center ${game.bgColor} p-2 rounded-xl`}>
            <motion.div
                key={game.id}
                className={`relative md:h-[70dvh] md:w-[70dvh] h-[45dvh] w-[45dvh] flex item-center rounded-lg p-8 group-hover:p-4 transition-all duration-500 text-2xl overflow-hidden`} >
                <div className={`h-full w-full bg-default-orange rounded-xl ${game.bgImage} bg-cover bg-center`}></div>
            </motion.div>
            <div className={`flex w-full md:flex-row flex-col gap-4 justify-between items-center p-12 font-bold text-3xl`}>
